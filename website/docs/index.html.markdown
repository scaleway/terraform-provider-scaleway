...
of the form xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx. under the *Tokens* subsection, but is labelled 
seperately as **access key**.

Your **token** can be generated by visiting the *Credentials* tab of the management panes.
Look in the *Tokens* subsection at the bottom of the page and create a new token if necessary
as the secret key is only shown once: when the key is created. Giving each token a 
