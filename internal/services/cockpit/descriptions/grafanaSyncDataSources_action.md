...
Grafana must be provisioned for the project before running this action. Prefer [`scaleway_cockpit_activate_grafana`](https://registry.terraform.io/providers/scaleway/scaleway/latest/docs/actions/cockpit_activate_grafana) (IAM first access). The legacy `scaleway_cockpit_grafana_user` resource also provisions Grafana but is deprecated.
