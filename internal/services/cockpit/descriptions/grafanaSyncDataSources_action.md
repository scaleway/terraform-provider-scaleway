...
Grafana must be provisioned for the project before running this action. With IAM authentication, access Grafana at least once for the project (for example `GET {grafana_url}/api/org` with header `X-Auth-Token: <secret_key>`).
