...
func TestAccWebhook_BasicAndUpdate(t *testing.T) {
	...
	subDomainName := temAccSubdomain("webhook-test", "tf-tem-wh")
		...
		Steps: []resource.TestStep{
			{
				...
				`, organizationID, webhookDomainName, subDomainName, testAccTEMProjectID, webhookName, initialEventTypes[0], initialEventTypes[1]),
			...
			},
			{
				...
				`, organizationID, webhookDomainName, subDomainName, testAccTEMProjectID, updatedWebhookName, updatedEventTypes[0]),
				Check: resource.ComposeTestCheckFunc(
					isWebhookPresent(tt, "scaleway_tem_webhook.webhook"),
