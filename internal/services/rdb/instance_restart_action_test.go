...
func TestAccActionRDBInstanceRestart_Basic(t *testing.T) {
	if acctest.IsRunningOpenTofu() {
		t.Skip("Skipping TestAccActionRDBInstanceRestart_Basic because action are not yet supported on OpenTofu")
	}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					isInstanceRestarted(tt, "scaleway_rdb_instance.main"),
				),
			},
		},
	})
...
func isInstanceRestarted(tt *acctest.TestTools, instanceResourceName string) resource.TestCheckFunc {
	return func(state *terraform.State) error {
		...
		if instance.Status != rdbSDK.InstanceStatusReady {
			return fmt.Errorf("instance %s is not ready after restart, status: %s", id, instance.Status)
		}
