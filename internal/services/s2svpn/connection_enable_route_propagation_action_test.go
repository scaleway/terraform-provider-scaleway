...
func TestAccActionS2SVPNConnectionEnableRoutePropagation_Basic(t *testing.T) {
	if acctest.IsRunningOpenTofu() {
		t.Skip("Skipping TestAccActionS2SVPNConnectionEnableRoutePropagation_Basic because action are not yet supported on OpenTofu")
	}
		...
		Steps: []resource.TestStep{
			{
				Config: `
					...
					resource "scaleway_s2s_vpn_gateway" "main" {
						name              = "tf-test-vpn-gateway-enable-route-prop"
						gateway_type      = "VGW-S"
					...
					resource "scaleway_s2s_vpn_customer_gateway" "main" {
						name        = "tf-test-customer-gateway-enable-route-prop"
					...
					resource "scaleway_s2s_vpn_routing_policy" "main" {
						name             = "tf-test-routing-policy-enable-route-prop"
					...
					resource "scaleway_s2s_vpn_connection" "main" {
						name                     = "tf-test-connection-enable-route-prop"
				...
				Check: resource.ComposeTestCheckFunc(
					...
					isRoutePropagationEnabled(tt, "scaleway_s2s_vpn_connection.main"),
				),
			},
		},
	})
...
func isRoutePropagationEnabled(tt *acctest.TestTools, connectionResourceName string) resource.TestCheckFunc {
	return func(state *terraform.State) error {
		...
		if !connection.RoutePropagationEnabled {
			return fmt.Errorf("connection %s has route propagation disabled, expected enabled", id)
		}
