...
func isPrivilegePresent(tt *acctest.TestTools, instance string, database string, user string) resource.TestCheckFunc {
	return func(state *terraform.State) error {
		...
		api, _, _, err := documentdb.NewAPIWithRegionAndID(tt.Meta, instanceResource.Primary.ID)
		if err != nil {
			return err
		}

		region, instanceID, userName, err := documentdb.ResourceDocumentDBUserParseID(userResource.Primary.ID)
		if err != nil {
			return err
		}

		_, databaseName, err := documentdb.ResourceDocumentDBDatabaseName(databaseResource.Primary.ID)
		if err != nil {
			return err
		}
