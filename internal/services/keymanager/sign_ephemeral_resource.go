	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			...
			"digest": schema.StringAttribute{
				Required:    true,
				Description: "Digest of the message to sign. The digest must be generated using the same algorithm defined in the key’s algorithm settings, and encoded in Base64.",
				Sensitive:   true,
			},
...
func (r *SignEphemeralResource) Open(ctx context.Context, req ephemeral.OpenRequest, resp *ephemeral.OpenResponse) {
	...
	data.Signature = types.StringValue(string(signResp.Signature))
