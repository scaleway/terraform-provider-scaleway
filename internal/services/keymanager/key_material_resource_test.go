		...
		Steps: []resource.TestStep{
			{
				Config: fmt.Sprintf(`
					...
					data "scaleway_key_manager_key" "main" {
						key_id     = scaleway_key_manager_key_material.main.id
						region     = local.region
						depends_on = [scaleway_key_manager_key_material.main]
					}
				`, createKeyMaterial()),
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr("data.scaleway_key_manager_key.main", "state", "enabled"),
...
func TestAccKeyMaterialResource_WithSaltWO(t *testing.T) {
	if acctest.IsRunningOpenTofu() {
		...
		Steps: []resource.TestStep{
			{
				...
				`,
				ExpectError: regexp.MustCompile(`Attribute \"key_material_wo_version\" must be specified when \"key_material_wo\"\s+is specified`),
			},
		},
	})
}
		...
		Steps: []resource.TestStep{
			{
				...
				`,
				ExpectError: regexp.MustCompile(`No attribute specified when one \(and only one\) of\s+\[key_material,key_material_wo\] is required`),
			},
		},
	})
}
		...
		Steps: []resource.TestStep{
			{
				...
				`,
				ExpectError: regexp.MustCompile("Attribute \"salt\" cannot be specified when \"salt_wo\" is specified"),
			},
		},
	})
}
		...
		Steps: []resource.TestStep{
			{
				...
				`,
				ExpectError: regexp.MustCompile("\"salt_wo_version\" must be specified when \"salt_wo\" is specified"),
			},
		},
	})
}
