...
// data (API keys, secrets, tokens) is leaked. Values must be anonymized (placeholders only).
...
func checkInteractionForLeaks(t *testing.T, cassettePath string, idx int, inter *cassette.Interaction) {
	...
	checkBodyForLeaks(t, cassettePath, idx, "response", inter.Response.Body)
	checkHeadersForLeaks(t, cassettePath, idx, "request", inter.Request.Headers)
	checkHeadersForLeaks(t, cassettePath, idx, "response", inter.Response.Headers)
}
...
func checkJSONForLeaks(t *testing.T, cassettePath string, idx int, part string, v any, path string) {
	...
	case map[string]any:
		for key, val := range x {
			...
			if placeholder, isSensitive := acctest.SensitiveFields[keyLower]; isSensitive {
