---
subcategory: "Key Manager"
page_title: "Scaleway: scaleway_key_manager_rotate_key"
---

# scaleway_key_manager_rotate_key (Action)

The [`scaleway_key_manager_rotate_key`](https://registry.terraform.io/providers/scaleway/scaleway/latest/docs/actions/key_manager_key_rotate_action) action is helpful to trigger a rotate key action on your key.

Refer to the Key Manager [documentation](https://www.scaleway.com/en/docs/key-manager/) and [API documentation](https://www.scaleway.com/en/developers/api/key-manager/) for more information.

<!-- action schema generated by tfplugindocs -->
## Schema

### Required

- `key_id` (String) ID of the key to rotate. Can be a plain UUID or a regional ID.

### Optional

- `region` (String) Region of the key. If not set, the region is derived from the key_id when possible or from the provider configuration.


