...
- `shard_count` - (Optional, Computed, Forces new resource) Number of shards for the deployment. This value is immutable and cannot be changed after creation.
...
- `public_network` - (Optional, Forces new resource) Public endpoint configuration. When defined, a public endpoint is created. Omitting this block creates a deployment without a public endpoint.
...
- `public_network` - Public endpoint information (only present if configured).
