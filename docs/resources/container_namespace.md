...
The `scaleway_container_namespace` resource allows you to manage Scaleway [Serverless Containers](https://www.scaleway.com/en/docs/serverless/containers/).
...
- `registry_namespace_id` - (Deprecated) The registry namespace ID of the namespace.

~> **Important:** A registry is no longer created alongside the namespace, it has to be handled separately. Therefore,
`registry_endpoint` and `registry_namespace_id` are no longer provided.
