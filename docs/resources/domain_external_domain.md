...
- `validation_token`: The validation token to add as a TXT record to prove domain ownership.

## Timeouts

`timeouts` block arguments:

- `create` - (Default `30m`) Create timeout.
- `read` - (Default `30m`) Read timeout.
- `delete` - (Default `30m`) Delete timeout.
- `default` - (Default `30m`) Default timeout.
