...
* `project_id` - (Defaults to [provider](../index.md#arguments-reference)
...
~> **Important:** Server-side encryption configuration IDs are
[regional](../guides/regions_and_zones.md#resource-ids), which means they are of
the form `{region}/{bucket-name}@{project_id}`, e.g.
