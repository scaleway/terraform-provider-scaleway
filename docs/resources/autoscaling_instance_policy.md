...
~> **Important:** The resource `scaleway_autoscaling_instance_policy` has been deprecated and will no longer be supported.
The Autoscaling API (v1alpha1) has been discontinued, this resource is no longer functional.
...
- `zone` - (Optional, Computed, Defaults to [provider](../index.md#arguments-reference) `zone`) The [zone](../guides/regions_and_zones.md#zones) in which the Instance policy exists.
