...
resource "scaleway_domain_record" "NS" {
  for_each = {
    ns0 = scaleway_domain_external_domain_validated.example.ns_servers[0]
...
- `ns_servers`: List of default NS servers for the domain once validated.
...
- `delete`: Custom duration for delete (no-op for this resource).
- `default`: Default timeout for operations.
