...
~> **Note** Major engine upgrades (especially with HA) can take longer than other updates. The default `timeouts.update` for this resource is **60 minutes**. Increase it further if needed. If an apply is interrupted after the new instance is created, `replaced_from_instance_id` keeps the previous instance ID so the next apply/refresh can finish deleting it.
...
- `replaced_from_instance_id` - Regional ID of the previous instance during a blue/green engine upgrade. Set while cleanup of the old instance is pending, empty otherwise.
