...
- **Prefer a dry run outside Terraform when unsure.** Terraform always runs the upgrade with endpoint migration (`WithEndpoints = true`). For a safer first pass, use one of these options instead, then migrate endpoints yourself and [import](#import) the new instance into Terraform:
  - In the [Scaleway console](https://www.scaleway.com/en/docs/managed-databases-for-postgresql-and-mysql/how-to/postgresql-upgrade-version/), choose **Upgrade only** (keeps the original endpoints on the source instance).
  - Or call the API / CLI without migrating endpoints, e.g. `scw rdb instance upgrade <instance-id> major-upgrade-workflow.upgradable-version-id=<version-id> major-upgrade-workflow.with-endpoints=false`, then migrate endpoints with [`scw rdb endpoint migrate`](https://www.scaleway.com/en/docs/managed-databases-for-postgresql-and-mysql/api-cli/migrating-endpoints/) when ready.
...
```bash
# List instances (filter by name when possible)
scw rdb instance list region=fr-par name=my-db -o json

# Inspect one instance (status, engine, endpoints)
scw rdb instance get instance-id=11111111-1111-1111-1111-111111111111 region=fr-par -o json
```
...
```bash
scw rdb instance delete instance-id=22222222-2222-2222-2222-222222222222 region=fr-par
```
...
```bash
# Remove the stale state entry (does not delete the cloud resource)
terraform state rm scaleway_rdb_instance.main

# Re-import using the new instance regional ID
terraform import scaleway_rdb_instance.main fr-par/11111111-1111-1111-1111-111111111111

# Reconcile configuration and dependent resources
terraform plan
terraform apply
```

If state already references the new instance ID, run `terraform plan` / `terraform apply` only — you may still need a second apply for dependent resources such as `scaleway_rdb_acl` that reference the instance ID.
