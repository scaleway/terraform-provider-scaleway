...
Use this procedure when a blue/green `engine` upgrade times out in Terraform. The Scaleway upgrade may still finish in the background (snapshot, restore, endpoint migration). **Do not re-apply an engine change** while an upgrade is still running.
